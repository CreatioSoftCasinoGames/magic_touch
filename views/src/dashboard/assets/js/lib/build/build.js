...
(function() {
  ...
  var handshake = function (data) {
    var data1 = JSON.parse(Protocol.strdecode(data));
    // console.log(data1)
    currentIndex++;
    tempStr += data1.data;
    // console.log('1669 lines ',JSON.stringify(data1));
    if (data1.last) {
      console.log('Chunks are merged !!')
      data = JSON.parse(tempStr);
      // data = JSON.parse(Protocol.strdecode(data));
      if (data.code === RES_OLD_CLIENT) {
      //console.log("initCallback 2222 " + RES_OLD_CLIENT);
        pomelo.emit('error', 'client version not fullfill');
        return;
      }

      if (data.code !== RES_OK) {
        console.log("initCallback 0000 " + data.code + " ssaasa " + RES_OK);
        ...
        send(obj);
          console.log("initCallback 1111 " + initCallback);
        if (initCallback) {
          initCallback(socket);
        }
